...
#: Valid notebook-HTML styles: the legacy layout or the Peel card (spec §17).
STYLES = ("legacy", "peel")
...
class RenderOptions:
    ...
    style: str = "legacy"  # notebook HTML style; "peel" opts into the new card
...
def set_display(style: str | None = None, **changes: object) -> RenderOptions:
    """Set process-wide display preferences.

    ``fd.set_display("peel")`` opts into the Peel notebook style;
    ``fd.set_display(mode="compact")`` etc. set individual fields. Returns the
    new options. Unknown fields raise ``TypeError``; an unknown mode or style
    raises ``ValueError`` so typos fail at configuration time, not render time.
    """
    ...
    if new.style not in STYLES:
        raise ValueError(f"unknown display style {new.style!r}; expected one of {STYLES}")
    _state["options"] = new
    return new
...
def get_display(**overrides: object) -> RenderOptions:
    ...
    opts = _state["options"]
    env_display = os.environ.get("FRESHDATA_DISPLAY")
    if env_display == "peel" and opts.style == "legacy":
        opts = replace(opts, style="peel")
    elif env_display and opts.mode == "auto" and env_display in MODES:
        opts = replace(opts, mode=env_display)
