...
| CSV exports safe to open in spreadsheets | **holds by default on all first-party CSV outputs** (`fd.clean_csv`, `freshdata clean` / `apply-plan`, the streaming CLI, review queues); opt out with `sanitize_formulas=False` / `--no-sanitize-formulas` | see [threat model](threat-model.md); `tests/test_csv_formula_sanitize.py` |
